...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"ringkasan" | "gaji" | "periode">(
    "ringkasan",
  );
  ...
  const handleUpdateHonor = async () => {
    if (editHonorModal) {
      await updateScheduleHonor(editHonorModal.id, editHonorModal.amount);
      ...
      {editHonorModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-sm">
            <CardHeader>
              <CardTitle>Ubah Nominal Honor</CardTitle>
              <p className="text-sm text-gray-500">
                Sesuaikan nominal honor untuk sesi ini sebelum divalidasi.
              </p>
            </CardHeader>
            <CardContent>
              <div>
